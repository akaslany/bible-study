...
if len(htmls)<146:errors.append(f"too few html={len(htmls)}")
for p in htmls:
...
for rel,markers in {'index.html':['성경별 공부','사도행전','최근 공부'],'books/index.html':['잠언','전도서','욥기','요한복음','사도행전'],'proverbs/index.html':['잠언 첫 번째 읽기'],'books/john/index.html':['요한복음 첫 번째 읽기','20개 기록 · 3장 없음','요한복음 두 번째 읽기'],'books/acts/index.html':['10장까지'],'studies/2026-08-16-acts-10/index.html':['사도행전 10장 묵상','현재까지 기록','인용 안내']}.items():
 p=SITE/rel
 if not p.exists():errors.append('missing page:'+rel);continue
 t=p.read_text()
 for m in markers:
  if m not in t:errors.append(f"{rel}:missing:{m}")
if len(list((SITE/'studies').glob('*/index.html')))!=136:errors.append('study html count')
if list(SITE.rglob('*.py')):errors.append('scripts leaked into site')
if errors:print('\\n'.join('FAIL: '+x for x in errors[:80]));sys.exit(1)
print(f"BUILT_SITE_TESTS_PASS html={len(htmls)} studies=136 internal_links=verified")
